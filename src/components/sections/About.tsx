...
const About = () => {
  ...
  return (
    <Section title="ABOUT">
      <div ref={aboutRef} className='space-y-4 sm:text-xl lg:font-light lg:text-lg lg:space-y-12'>
        {
          about.description.map((s, i) => (
            <p className='flex flex-wrap gap-1' key={i}>
              {s.split(' ').map((w, idx) => (
                w.includes('HTML') || w.includes('CSS') || w.includes('JavaScript') || w.includes('React') || w.includes('TailwindCSS') ? (
