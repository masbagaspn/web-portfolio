        ...
        { experiences.map((exp, idx) => (
          <Card key={idx}>
            <div className='flex flex-col sm:flex-row sm:gap-8 lg:flex-col'>
              <span className='text-xs text-neutral-100/50 uppercase sm:w-1/4 sm:text-sm lg:w-full'>{exp.date.joinedDate} — {exp.date.endDate}</span>
              <div className='flex flex-col mt-1 font-semibold text-lime-400 gap-1.5 sm:w-3/4 sm:text-2xl sm:mt-0 lg:mt-4 lg:w-full'>
                <h3>{exp.as} ·
                  <span>{` ${exp.company}`}</span>
                </h3>
                {exp?.contract !== null && (
                  <span className='text-neutral-100/50 sm:text-xl font-light'>{exp.contract}</span>
                )}
                <p className='mt-8 font-light text-sm text-neutral-100 lg:text-base'>{exp.description}</p>
